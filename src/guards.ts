...
export async function runGuard(
  g: AST.GuardDecl, cwd: string
): Promise<GuardResult> {
  const files = await fg(g.scope, {
    cwd, absolute: true, ignore: DEFAULT_IGNORE
  });
  const passed:   string[] = [];
...
async function checkAssert(
  ...
  // ── fn_exists — AST-accurate ─────────────────────────────────────────────
  // Old version used .includes() which gave false positives for comments.
  // This version uses the real AST so comments cannot fool it.
  if (a.type === "fn_exists") {
    const fnName = a.sig
      .replace(/\(.*\)/, "")
      .replace(/:\s*\S+/, "")
    ...
    for (const file of files) {
      ...
      if ([".ts", ".tsx", ".js", ".jsx"].includes(ext)) {
        ...
        if (found) return {
          ok: true,
          msg: `✓ fn '${fnName}' confirmed in AST (${path.relative(cwd, file)})`
        };
        continue;
      }
      ...
      if (ext === ".py") {
        ...
        if (findFn(p.parse(src).rootNode, fnName,
      ...
      // Fallback — word-boundary regex for unsupported languages
      const re = new RegExp(
        `(?:function|def|func|fn|sub)\\s+${escRe(fnName)}\\s*\\(`, "m"
      );
      if (re.test(src)) return {
        ok: true,
        msg: `✓ fn '${fnName}' found via pattern match`
      };
    }
  ...
  if (a.type === "no_import_from") {
    const bad = files.filter(f => {
      ...
      return s.includes(`from "${a.path}"`) ||
             s.includes(`from '${a.path}'`);
    });
    return {
      ok: bad.length === 0,
      msg: bad.length === 0
        ? `✓ no file imports from '${a.path}'`
    ...
    for (const f of files) {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i]!.includes(a.near)) {
          ...
          if (win.includes(a.text))
            bad.push(`${path.relative(cwd, f)}:${i + 1}`);
        }
      }
    }
    return {
...
function findFn(
  ...
  if (types.includes(node.type) &&
      node.childForFieldName("name")?.text === name) return true;
